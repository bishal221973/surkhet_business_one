...
class DateConverter
{
    ...
    public static function getDaysInMonth($year, $month)
    {
        ...
        // Find the index in the $_bs array
        $index = array_search($year, array_column(self::$_bs, 0));
        if ($index === false) {
            throw new \InvalidArgumentException("Year not supported in DateConverter.");
        }

        // Return number of days in that month
        return self::$_bs[$index][$month];
    }

    public static function getSaturdaysInMonth($bsYear, $bsMonth)
{
    $totalDays = self::getDaysInMonth($bsYear, $bsMonth);
    $saturdayCount = 0;

    for ($day = 1; $day <= $totalDays; $day++) {
        $engDate = self::nep_to_eng($bsYear, $bsMonth, $day);
        $weekday = $engDate['day']; // Nepali day name from nep_to_eng

        // Check if it's Saturday ("शनिबार")
        if ($weekday === "शनिबार") {
            $saturdayCount++;
        }
    }
...
public static function getDayCountInMonth($bsYear, $bsMonth, $dayName)
{
    $totalDays = self::getDaysInMonth($bsYear, $bsMonth);
    $count = 0;
